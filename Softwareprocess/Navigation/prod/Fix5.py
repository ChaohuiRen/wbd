...
#             print "the logfile's path is:",Path
    ...
    def getSightings(self,assumedLatitude=None,assumedLongitude=None):
        ...
        if 'd' in assumedLatitude:
            subassumedLatitude1=assumedLatitude.split('d')
            if float(subassumedLatitude1[1])<0.0 or float(subassumedLatitude1[1])>=60.0:
                raise ValueError("Fix.getSightings:  the value of assumedLatitude's minute portion is invalid")
            list1=list(assumedLatitude)
            try:
                A=int(list1[0])
                assumedLatitude='0d0.0'
            except:
                if list1[0]=='N' :
                    doublesubassumedLatitude1=subassumedLatitude1[0].split('N')
#                     print doublesubassumedLatitude1[1]
                    
                    if int(doublesubassumedLatitude1[1])<0 or int(doublesubassumedLatitude1[1])>= 90:
                        raise ValueError("Fix.getSightings:  the value of assumedLatitude's degree portion is invalid")
                if list1[0]=='S':
                    doublesubassumedLatitude2=subassumedLatitude1[0].split('S')
                    if int(doublesubassumedLatitude2[1])<0 or int(doublesubassumedLatitude2[1])>= 90:
                        raise ValueError("Fix.getSightings:  the value of degree portion is invalid")
                if list1[0]!='N' and list[0]!='S':
                    raise ValueError("Fix.getSightings:  the name of assumedLatitude is invalid")  


                    
            if 'N' in assumedLatitude:
                subassumedLatitude2=assumedLatitude.split('N')
            if 'S' in assumedLatitude:
                subassumedLatitude3=assumedLatitude.split('S')
        else:
            raise ValueError("Fix.getSightings:  no separator d in assumedLatitude")
        
        if 'd' in assumedLongitude:
            subassumedLongitude1=assumedLongitude.split('d')
            if float(subassumedLongitude1[1])<0.0 or float(subassumedLongitude1[1])>=60.0:
                raise ValueError("Fix.getSightings:  the value of assumedLongitude's minute portion is invalid") 
            if int(subassumedLongitude1[0])<0 or int(subassumedLongitude1[0])>=360:
                raise ValueError("Fix.getSightings:  the value of assumedLongitude's degree portion is invalid")
        else:
            raise ValueError("Fix.getSightings:  no separator d in assumedLongitude")
...
#             print component1[0]
#             print component1[1]
            if component1[0]=="04/17/17" and component1[1]=='23':
...
#         print GHAobservation1
        
        
        geolati11=self.angle.setDegreesAndMinutes(geolati1)
#         print('***********')
#         print ('geolati11:',geolati11)
        geolati111=geolati11*math.pi/180.0
        numberassumedLongitude=self.angle.setDegreesAndMinutes(assumedLongitude)
        if 'N' in assumedLatitude:
             
            numberassumedLatitude =self.angle.setDegreesAndMinutes(subassumedLatitude2[1])
            numberassumedLatitudeh=numberassumedLatitude*math.pi/180.0
#             print numberassumedLatitude 
        if 'S' in assumedLatitude:
            numberassumedLatitude =self.angle.setDegreesAndMinutes(subassumedLatitude3[1])
            numberassumedLatitudeh=numberassumedLatitude*math.pi/180.0
#             print numberassumedLatitude 
            
        LHA=float(GHAob1)+float(numberassumedLongitude)
#         print numberassumedLongitude
#         print "lha:"
#         print LHA
        LHAA=LHA*math.pi/180.0
        
        A=sin(geolati111)*sin(numberassumedLatitudeh)
        
#         print ('A:',A)

        B=cos(geolati111)*cos(numberassumedLatitudeh)*cos(LHAA)
#         print ('B:',B)
        C=A+B
#         print C
        correctedaltitude=arcsin(C)
#         print('correctedaltitude:',correctedaltitude)
        correctedaltitudeD=correctedaltitude*180.0/math.pi
#         print correctedaltitudeD
        
#         print correctedaltitudeD
#         print adjusteAltitude[0]
#         print adjustealtitude[0]
#         print ('**************')
        
        
        
        distanceadjustment=correctedaltitudeD-adjusteAltitude[0]
#         print distanceadjustment
        distanceadjustmenth=int(round(distanceadjustment*60,0))
#         print distanceadjustmenth
#         distanceadjustmenth=distanceadjustment*math.pi/180.0
#         self.angle.setDegrees(float(distanceadjustment))
#         distanceAdjustment=self.angle.getString()
#         print distanceAdjustment
#         distanceadjustmenth=distanceadjustment*math.pi/180.0
#         azimuthadjustment=acos((sin(geolati11)-sin(numberassumedLatitude)*sin(float(distanceadjustment)))/(cos(numberassumedLatitude)*cos(float(distanceadjustment))))
        D=sin(geolati111)
#         print('D:',D)
        E=sin(numberassumedLatitudeh)*(A+B)
        numerator=D-E
#         print('numerator:', numerator)
        denominator=cos(numberassumedLatitudeh)*cos(correctedaltitude)
#         print('denominator:', denominator)
        F=arccos(numerator/denominator)
#         print('F:',F)
        azimuthadjustment=F*180.0/math.pi
#         HR=azimuthadjustment.real

#         print HR
#         print azimuthadjustment
        self.angle.setDegrees(float(azimuthadjustment))
        azimuthAdjustment=self.angle.getString()
#         print distanceAdjustment
#         print azimuthAdjustment
#         
#         
...
#         print SHA2,geolati2,GHA11,GHA22,gha2, second,GHAaries2,GHAob2,GHAobservation2,(second/3600.0)
        ...
        abc=0   
        while  abc<=1 :
            if bb[abc].firstChild.data != 'Unknown' :
                s.write('LOG:'+formatTime+'-06:00 '+str(bb[abc].firstChild.data)+' '+str(dd[abc].firstChild.data)+' '+str(tt[abc].firstChild.data)+' '+str(adjustealtitude[0])+' '+str(geolati1)+' '+str(GHAobservation1)+' '+str(assumedLatitude)+' '+str(assumedLongitude)+' '+str(azimuthAdjustment)+' '+str(distanceadjustmenth)+'\n')
            abc+=1
...
#         s.write('LOG:'+formatTime+'-06:00 End of sighting file: '+str(self.sightingFile)+'\n')
        ...
        approximateLatitude=numberassumedLatitude+(distanceadjustmenth*cos(F))/60
#         print approximateLatitude
        if approximateLatitude<0:
            self.angle.setDegrees(abs(approximateLatitude))
            Approx=self.angle.getString()
            ApproximateLatitude='S'+Approx
        if approximateLatitude>0:
            self.angle.setDegrees(approximateLatitude)
            Approx=self.angle.getString()
            ApproximateLatitude='S'+Approx
        approximateLongitude=numberassumedLongitude+(distanceadjustmenth*sin(F))/60
        self.angle.setDegrees(approximateLongitude)
        ApproximateLongitude=self.angle.getString()
#         print ('approximateLatitude:',ApproximateLatitude)
#         print ('approximateLongitude:', ApproximateLongitude)
        tuple1=(ApproximateLatitude,ApproximateLongitude)
        
        s.write('LOG:'+formatTime+'-06:00 Sighting errors: '+'Sighting errors:  1'+'\n')
        s.write('LOG:'+formatTime+'-06:00 Approximate latitude:  ' +str(ApproximateLatitude)+'  '+'Approximate longitude:  '+str(ApproximateLongitude)+'\n') 
        s.close()
        return tuple1  
